class << ActiveRecord::Base
  def has_ancestry options = {}
    ...
    scope :ordered_by_ancestry, lambda { reorder("(CASE WHEN #{quoted_ancestry_column} IS NULL THEN 0 ELSE 1 END), #{quoted_ancestry_column}") }
    scope :ordered_by_ancestry_and, lambda { |order| reorder("(CASE WHEN #{quoted_ancestry_column} IS NULL THEN 0 ELSE 1 END), #{quoted_ancestry_column}, #{order}") }
